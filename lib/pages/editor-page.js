...
export default class EditorPage extends BaseContainer {
	...
	uploadMediaAndInsert( fileDetails ) {
		...
		const insertSelector = webdriver.By.css( '.dialog__action-buttons button.is-primary' );
		...
		const insertButton = driver.findElement( insertSelector );
		driver.wait( until.elementIsEnabled( insertButton ), this.explicitWaitMS, 'The insert image button is not enabled' );
		insertButton.click();
		d.fulfill( true );
		return d.promise;
	}
